...
type StorageGroups interface {
	...
	ListStorageGroups(context.Context, ...ListStorageGroupsFilters) ([]*StorageGroup, error)
...
type ListStorageGroupsFilters struct {
	CPCodeID            int
	StorageGroupPurpose string
}

func (p *storage) ListStorageGroups(ctx context.Context, filters ...ListStorageGroupsFilters) ([]*StorageGroup, error) {
	logger := p.Log(ctx)
	...
	getURL := "/storage/v1/storage-groups"
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, getURL, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to create ListStorageGroups request: %w", err)
	}

	q := req.URL.Query()
	if len(filters) > 0 {
		if filters[0].CPCodeID > 0 {
			q.Add("cpcodeId", strconv.Itoa(filters[0].CPCodeID))
		}
		if filters[0].StorageGroupPurpose != "" {
			q.Add("storageGroupPurpose", filters[0].StorageGroupPurpose)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("ListStorageGroups request failed: %w", err)
	}
...
// MANUAL EDITS:
// Renamed CpcodesItems => StorageGroupCpcodesItems
